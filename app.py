...
CORS(app, origins=["https://*tendoledu.com/*", "http://localhost*"])
...
async def post_signup_form_to_firestore(email, expertise, page_name):
    async with Aiogoogle() as google:
            ...
            firestore.projects.databases.documents.createDocument(
                ...
                json=dict(
                    fields={
                        ...
                        "pageName": {"stringValue": page_name},
                        "expertise": {"stringValue": expertise},
                    }
                ),
...
async def signup_handler(request):
    ...
    if valid is not True:
        ...
        await post_signup_form_to_firestore(
            form.email.data, form.expertise.data, form.page_name.data
        )
